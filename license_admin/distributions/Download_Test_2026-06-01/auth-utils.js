...
// ── License guard: intercept 402 responses from any API call ─────────────────
// When the server returns 402 with error:"license_required", redirect to the
// activation page immediately regardless of which page made the request.
(function () {
        ...
        if (response.status === 402) {
            try {
                ...
                if (body.error === 'license_required') {
                    ...
                    if (page !== 'license_activation.html') {
                        window.location.replace('license_activation.html');
                    }
                }
            } catch (_) { /* non-JSON 402 — ignore */ }
        }
        return response;
    };
})();
